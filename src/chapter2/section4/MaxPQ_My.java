...
public class MaxPQ_My<Key extends Comparable<Key>> implements Iterable<Key> {
	 ...
	 */
	public MaxPQ_My(int initCapacity) {
	 ...
	 */
	public MaxPQ_My(int initCapacity, Comparator<Key> comparator) {
	 ...
	 * @param comparator the order in which to compare the keys
	 */
	public MaxPQ_My(Comparator<Key> comparator) {
		this(1, comparator);
	}
	...
	private class HeapIterator implements Iterator<Key> {
		...
		public HeapIterator() {
			if (comparator == null)
				copy = new MaxPQ_My<>(size());
			else
				copy = new MaxPQ_My<>(size(), comparator);
			for (int i = 1; i <= n; i++)
				copy.insert(pq[i]);
		}
